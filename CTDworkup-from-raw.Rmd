...
[Code for this document](https://github.com/cbonsell/ble-instrumentation/blob/master/CTDworkup-from-raw.Rmd)


[CTD-QAQC.Rmd](https://github.com/cbonsell/ble-instumentation/blob/master/CTD-QAQC.Rmd) is a cleaned-up version that just returns derived datasets and final plots


##Import CTD data

First few rows:
```{r CTD_data, echo=FALSE, message=FALSE, warning=FALSE}
source('scr/CTDdata_import.R') #Star Oddi, RBR, and TCM data
datatable(head(RBR), caption="RBR")
datatable(head(StarOddi), caption="StarOddi")
datatable(head(TCM_Temp), caption="Tilt Current meter")
```
...
datatable(head(YSI), caption="YSI")
```
...
```{r chose_datasets, echo=FALSE, message=FALSE, warning=FALSE, fig.width=8, fig.height=6, fig.fullwidth=TRUE}
...
RBR_final<-RBR_flagged%>%mutate(flags_CondSal=case_when(flag==T~"QM", 
                                                        anom==T~"INV"))
RBR_final$flags_CondSal[is.na(RBR_final$flags_CondSal)] <- "OK"

SO_final<-SO_flagged%>%mutate(flags_CondSal=case_when(flag==T~"QM", 
                                                      anom==T~"INV"), 
                                flags_Temperature=case_when(flag==T & Station=="EELD1" ~ "QM"))
SO_final$flags_CondSal[is.na(SO_final$flags_CondSal)] <- "OK"
SO_final$flags_Temperaturel[is.na(SO_final$flags_Temperature)] <- "OK"

ggplot(RBR_final, aes(DT,Salinity))+
  geom_point(size=1.1, alpha=0.5, aes(color=flags_CondSal))+
  facet_wrap(~Station)+
  theme(legend.position="bottom")+
  theme_cowplot()
...
ggplot(SO_final, aes(DT,Temp_C))+
  geom_point(size=1.1, alpha=0.5, aes(color=flags_Temperature))+
  facet_wrap(~Station)+
  theme(legend.position="bottom")+
  theme_cowplot()
...
```{r finalize, echo=FALSE, warning=FALSE, message=FALSE, fig.width=11, fig.height=8, fig.fullwidth=TRUE}
...
SO_final<-SO_final%>%select(Station, DT, Temperature=Temp_C, Salinity=Sal, Conductivity=Cond_mspercm,
                             Speed.of.sound=SoundVel_mpers, Instrument,flags_CondSal,flags_Temperature,calibrated)

# plots
SO_plot<- SO_final %>% filter(flags_CondSal=="OK")
...
RBR_final %>%  filter(flags_CondSal=="OK") %>% 
ggplot(aes(DT,Salinity))+
  geom_point(aes(color=Station), alpha=0.5)+
  geom_point(data=SO_plot, aes(DT,Salinity, color=Station), alpha=0.5)+
  theme_cowplot()

RBR_final %>%  filter(flags_CondSal=="OK") %>% 
ggplot(aes(DT, Conductivity))+
  geom_point(aes(color=Station), alpha=0.5)+
  geom_point(data=SO_plot, aes(DT,Conductivity, color=Station), alpha=0.5)+
  theme_cowplot()
