...
```{r inst_compare, echo=FALSE, message=FALSE, warning=FALSE, results='asis', fig.width=11, fig.height=8, fig.fullwidth=TRUE}
library(knitr)
...
datatable(Itable)
###
kable(Itable, caption="Diffs b/n instruments: TEMP and SAL")

Inst.compare%>%filter(Station=="EELD1"|Station=="EWLD2"|Station=="JALD2"|Station=="KALD2")%>%
...
Stable<-Inst.compare%>%inner_join(YSI, by=c("DT","Station"))%>%
  ...
  filter(!is.na(DT))
kable(Ttable, caption="Diff in YSI and instruments: TEMPERATURE")
kable(Stable, caption="Diff in YSI and instruments: SALINITY")
```
...
SO2%>%filter(Station=="KALD1"|Station=="KALD1_calibrated"|Station=="KALD1_calibratedCond"|Station=="KALD2")%>%
  ggplot(aes(DT,Sal))+
...
SO2%>%filter(Station=="EWLD1"|Station=="EWLD1_calibrated"|Station=="EWLD1_calibratedCond"|Station=="EWLD2")%>%
  ggplot(aes(DT,Sal))+
  geom_point(size=1.1, alpha=0.5, aes(color=Station))+
...
#give cond and sal columns invalid codes
RBR_final[,3:12][is.na(RBR_final[,3:12])] <- -9999
...
SO_final[,3:7][is.na(SO_final[,3:7])] <- -9999
