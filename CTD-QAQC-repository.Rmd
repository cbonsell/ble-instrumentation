...
StarOddi <- read_csv("StarOddi_condensed_raw.csv") #calibrations were made, so read in raw data for transformation
...
calibrated <- left_join(calibrated_cond,KALD1c) %>% 
  select(station,date_time, temperature, conductivity) %>% 
...
StarOddi <- StarOddi%>%filter(station !="KALD1"& station!="EWLD1")%>%
  ...
  select(station, date_time, temperature, conductivity, salinity, recalibrated_parameters)
...
```{r freeze_flag, message=FALSE, warning=FALSE}
...
```{r final_plots, warning=FALSE, message=FALSE}
#plots
...
Write csv
```{r derived-data}
write.csv(CTD, "results/BLE LTER insituCTD 2018to2019.csv")
